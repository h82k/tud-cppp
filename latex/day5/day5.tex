...
Der Aufruf der Toolchain (Compiler, Linker und Flashprogramm) für die Embedded-Entwicklung ist in einem Makefile integriert, das in Eclipse durch den Buildprozess automatisch gestartet wird.

\hints{
	\item
	Um ein Programm auf den Mikrocontroller zu flashen, muss zuvor der Schiebeschalter auf dem Board auf \textbf{PROG} geschaltet werden.
	...
	\item
	Pool-PCs: Das USB-Kabel muss am Hub in der Buchse stecken, die mit \emph{Data} beschriftet ist.
	\item
	Variablen dürfen in dieser Übung im Gegensatz zu C++ nur am Anfang eines Anweisungsblocks deklariert werden.
...
\begin{center}
	\includegraphics[width=0.25\textwidth]{figures/7seg_e}
\end{center}
...
\begin{lstlisting}
void initADC(void)                // initialises the ADCs
int getADCValue(int channel)   // either reads the value from channel 1 or 2
\end{lstlisting}

\hints{
	\item 
	Einige Funktionen aus \ref{exercise7SegmentUtil} kannst du hier wieder verwenden.
	\item
	Der Wert für \texttt{ADSR} besteht aus 16 Bits (0110 11xx xxxy yyyy\textsubscript{b}), wobei xxxxx für den Startkanal der Konvertierung und yyyyy für den Endkanal steht. Für unsere Zwecke nehmen diese beiden 5\,Bit Blöcke immer entweder 00001\textsubscript{b} = 1 (AN1) oder 00010\textsubscript{b} = 2 (AN2) an.
...
Zur Vereinfachung wurden bereits im Projekt für diese Aufgabe Definitionen vorgegeben, sodass die Pins über einfache Namen angesteuert werden können. Die Namen entsprechen den Pins, wie sie im Datenblatt (\textit{<SVN>/Doku/Display\_AV128641YFBY-WSV.pdf}) ab Seite 10 zu finden sind.
...
\end{enumerate}
% TODO referenz auf das datenblatt
...
\begin{lstlisting}
void lcd_clear();     // clears the frame buffer, setting all pixels to 0.
void lcd_drawPixel(int x, int y, int black);    // sets (black == 1) or clears (black == 0) a pixel in the frame buffer, does nothing, if any input parameter is invalid.
void lcd_flush();    // displays the frame buffer on the LCD
\end{lstlisting}
...
\hints{
	...
	\item
	Für die Funktion \textit{lcd\_drawPixel} werden die Bitoperationen AND und OR sowie der Shift-Operator benötigt.
