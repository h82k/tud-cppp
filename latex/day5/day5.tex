...
Es empfiehlt sich, für dieses Signal eine eigene Funktion zu schreiben (\lstinline{void lcd\_sendEnable(void)}).
\end{enumerate}
...
\hints{
	\item Denke daran, \lstinline{LCD\_PIN\_E} zu Beginn des Programms mit 0 zu initialisieren.
	\item Das Reset-Signal (\lstinline{LCD\_PIN\_RESET}) muss immer 1 (deaktiviert) sein.
}
...
Die Vorlage enthält bereits ein Array \lstinline{lcd\_buffer}, das als Framebuffer fungieren soll.
...
\hints{
	...
	\item Für die Funktion \lstinline{lcd\_drawPixel} werden die bitweisen Operationen AND und OR sowie der Shift-Operator benötigt.
