...
\hints{
	\item Über \texttt{std::make\_shared(<Konstruktorparameter>)} kann man auch einen Smart Pointer erhalten.
}
...
\hints{
	\item \texttt{p} wird in diesem Fall nur deshalb als \texttt{const} Referenz übergeben, da es verhältnismäßig aufwändig ist, einen Smart Pointer zu kopieren.
	Beachte, dass im obigen Fall der Smart Pointer selbst \texttt{const} ist, und nicht das Objekt, worauf er zeigt.
}
...
\hints{
	\item Faustregel: Wenn ein Objekt ein anderes kontrolliert oder enthält, verwende Shared Pointer vom Container/Besitzer zum anderen Objekt und einen Weak Pointer für die umgekehrte Richtung.
}
