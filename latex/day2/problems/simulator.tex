...
Beispielsweise wird deine \lstinline{Building}-Klasse zwei Getter für \lstinline{Floor}s enthalten:
\begin{lstlisting}
class Building {
public:
    // ...
...
Du kannst das Element, auf den der Iterator zeigt, durch den Dereferenzierungsoperator (\lstinline{*iter}) holen.
...
Wenn die Person in ihrem Zielstockwerk angekommen ist, wird sie aus \lstinline{containedPeople} gelöscht zu \lstinline{arrived} hinzugefügt.
...
\hints{
	...
	Die Methode \lstinline{erase} gibt einen Iterator auf das Element hinter dem gelöschten zurück.
}
