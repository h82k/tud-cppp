...
void ball_init(ballData* ball) {
	// TODO improve initial ball direction
...
void ball_reflect(ballData* ball, int horizontal) {
	if (horizontal) {
		ball->xDir = - ball->xDir;
	}
	else {
...
void check_collision(ballData* ball, playerData* player) {
	...
	// if not check if one player lost the game by letting the ball touch the left or right wall
	if (ball->xPos == 0) {
		player->pointsLeft++;
		ball_init(ball);
		return;
