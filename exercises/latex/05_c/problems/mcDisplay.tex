...
Im letzten Teil dieser Aufgabe färbst du den Bildschirm meiner Farbe deiner Wahl.
\begin{enumerate}
\item 
Beginne wieder mit einer leeren \lstinline|main|-Funktion.
\item 
Binde die Header \filename{lcd.h} (aus \winIdeaGroupName{lib}) und \filename{display.h} ein und füge den folgenden Initialisierungscode für das Display in \lstinline|main| ein:
\begin{lstlisting}
init();
reset();
setupLCD();
