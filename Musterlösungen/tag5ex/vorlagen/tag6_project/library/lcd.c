...
void lcd_drawRect(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t black, uint8_t filled) {
	...
	if (!filled) {
		lcd_drawLine(x,     y,     x+w-1, y, black);
		lcd_drawLine(x+w-1, y,     x+w-1, y+h-1, black);
		lcd_drawLine(x+w-1, y+h-1, x,     y+h-1, black);
		lcd_drawLine(x,     y+h-1, x,     y, black);
		return;
	}
